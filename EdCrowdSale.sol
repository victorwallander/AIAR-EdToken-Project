...
contract Crowdsale is OwnableWithAdmin {
  ...
  **/
  function fiatTransfer(address _recipient, uint256 _tokenAmount, uint256 _weiAmount) onlyOwnerOrAdmin public{
    ...
    // calculate token amount to be created after rate update
    uint256 _checkTokenAmount = _calculateTokens(_weiAmount);

    //Check value
    require(_checkTokenAmount == _tokenAmount);
...
/**
 * @title EDCrowdsale
 * @notice Contract is payable and owner or admin can 
 * also buy and mint tokens directly to the receiving party’s wallet. 
 * Tokens will be minted directly to the buyers wallet.
 *
*/
contract EDCrowdsale is Crowdsale {
  constructor(   
    uint256 _startTime, 
    uint256 _endTime,  
    address _wallet, 
